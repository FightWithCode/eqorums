...
class AccountManagerView(APIView):
	...
	def get(self, request):
		try:
			...
			response['profile_photo'] = profile_obj.profile_photo.url if str(profile_obj.profile_photo) not in ["", "None", "null"] else None
			clients_list = []
			try:
	...
	def get(self, request):
		try:
			...
			for i in hiring_member_objs:
				...
				temp_dict["email"] = i.email
				temp_dict["profile_photo"] = i.profile_photo.url if str(i.profile_photo) not in ["", "None", "null"] else None
				if i.job_title:
			...
			for can in candidates_objs:
				...
				try:
					...
					for e in eval_objs:
						...
						t_data["given_by_pic"] = e.given_by.profile_photo.url if e.given_by.profile_photo not in ["", "None", "null"] else None
	...
	def get(self, request, uuid):
		...
		try:
			...
			try:
				...
				if candidate_obj.temp_profile_photo:
					data["profile_photo"] = {}
